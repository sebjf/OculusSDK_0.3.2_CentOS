...
void HMDDeviceFactory::EnumerateDevices(EnumerateVisitor& visitor)
{
    ...
    MonitorInfo* fakeDK2MonitorInfo = new MonitorInfo();
    memcpy(fakeDK2MonitorInfo->manufacturer_code, "OVR", 3);
    fakeDK2MonitorInfo->product_code = 3;
    ...
    for (int iscres = screen->noutput - 1; iscres >= 0; --iscres) {
        ...
        if(iscres == (screen->noutput - 1)){
        	//pretend the first monitor is a dk2
        	mi  = fakeDK2MonitorInfo;
        }
